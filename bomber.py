...
╱╱╱╱╱╱╱╱╱╱╱╱╰━━╯" 
			...
			def update():
				...
				if a=="y":
					os.system("cd && rm -rf spymer && git clone https://github.com/romanmajorov/armagedon && cd bomber && sh install.sh")
					exit()
				else:
			...
			def n_send(phone, count, proxies):
				...
				print(Fore.GREEN+"\nSpamm  {} stopped.".format(phone, count)+Style.RESET_ALL)
				exit()
			...
			def main():
				...
				while True:
					...
					if proxy == "localhost":
						print(Fore.YELLOW+"Bro, use a proxy!"+Style.RESET_ALL)
					print("1) SMS .")
					print("2) Update proxy")
					print("3) Update spymer")
					print("4) Exit")
					input1 = input(Fore.BLUE+"Select an action: "+Style.RESET_ALL)
					if input1 == "1":
						clear()
						logo()
						print(info)
						print("Select an action:")
						print("1. Start spymer in one number")
						print("2. Start spamm in number from list txt ")
		...
		os.system('cls' if os.name=='nt' else 'clear')
		print("Please, press 'Enter' to download missing files...")
		input()
